...
Module Calendar. Definition id : go_string := "github.com/mjschwenne/grackle/example.Calendar"%go. End Calendar.
Module Status. Definition id : go_string := "github.com/mjschwenne/grackle/example.Status"%go. End Status.
Module Person. Definition id : go_string := "github.com/mjschwenne/grackle/example.Person"%go. End Person.
Module Event. Definition id : go_string := "github.com/mjschwenne/grackle/example.Event"%go. End Event.
Module TimeStamp. Definition id : go_string := "github.com/mjschwenne/grackle/example.TimeStamp"%go. End TimeStamp.
...
Definition msets' : list (go_string * (list (go_string * val))) := [(Calendar.id, []); (ptrT.id Calendar.id, []); (Status.id, []); (ptrT.id Status.id, []); (Person.id, []); (ptrT.id Person.id, []); (Event.id, []); (ptrT.id Event.id, []); (TimeStamp.id, []); (ptrT.id TimeStamp.id, [])].
...
#[global] Instance info' : PkgInfo example.example :=
  {|
    ...
    pkg_msets := msets';
    pkg_imported_pkgs := [code.github_com.tchajed.marshal.marshal; code.github_com.goose_lang.std.std; code.github_com.goose_lang.primitive.primitive];
  |}.
